...
def evaluate_policy(policy_name: str, policy, env: SchedulerEnv, max_steps: int) -> dict:
    print(f">>> 开始评估: {policy_name}", flush=True)
    ...
    print(f">>> 完成评估: {policy_name} -> {result}", flush=True)
    return result


def main():
    print(">>> compare_baselines.py 已启动", flush=True)
    ...
    print(f">>> PPO 模型路径: {model_path}", flush=True)
    print(f">>> 模型文件是否存在: {model_path.with_suffix('.zip').exists()}", flush=True)
    ...
    csv_path = output_dir / "baseline_comparison_v2.csv"
    df.to_csv(csv_path, index=False, encoding="utf-8-sig")

    print(f"\n>>> 对比结果已保存到: {csv_path}", flush=True)
